...
function TicketManager({ apiBase }) {
  ...
  const filterQuery = useMemo(() => {
    ...
    if (filters.createdByEmail) params.set('createdByEmail', filters.createdByEmail)
    ...
    try {
      ...
      const payload =
        action.type === 'assign'
          ...
          : { status: action.value, resolutionNotes: action.value, reason: action.value }
    ...
    <section className="ticket-manager">
      <header>
        <div>
          ...
          <p className="lead">Assign technicians, update status, and monitor issues.</p>
        </div>
              ...
              tickets.map((ticket) => (
                <tr key={ticket.id}>
                  <td>
                    ...
                    <span className="muted">{ticket.resourceId || ticket.location}</span>
                  </td>
      ...
      {action.id ? (
        <div className="action-panel">
          ...
          {action.type === 'assign' ? (
            <input
              value={action.value}
              onChange={(event) => setAction((prev) => ({ ...prev, value: event.target.value }))}
